defmodule SymphonyElixir.AppServerTest do
  ...
  test "app server treats completed turns with failed status as errors" do
    test_root =
      Path.join(
        System.tmp_dir!(),
        "symphony-elixir-app-server-completed-failed-#{System.unique_integer([:positive])}"
      )
    ...
    try do
      ...
      while IFS= read -r line; do
        ...
        case "$count" in
          ...
          3)
            ...
            ;;
          4)
            printf '%s\\n' '{"method":"error","params":{"message":"model unavailable"}}'
      ...
      issue = %Issue{
        id: "issue-completed-failed",
        identifier: "MT-1002",
        title: "Capture failed completed turn",
        description: "Ensure failed completed turns are not treated as success",
        state: "In Progress",
      ...
      log =
        capture_log(fn ->
          assert {:error, {:turn_completed, "failed", %{"turn" => %{"status" => "failed"}}}} =
                   AppServer.run(workspace, "Capture failed completion", issue, on_message: on_message)
        end)
